...
router.post('/add', (req, res, next) => {
  ...
  let { emailAddress, artists, releaseName} = req.body;
...
router.post('/search', (req, res) => {
  ...
  RecordCrate.findAll({ where: { emailAddress: { [Op.like]: '%' + emailAddress + '%' } } })
    .then(recordCrates => {
      console.log("recordCrates:" + JSON.stringify(recordCrates));
      return res.status(200).json({
        success: true,
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    console.log("deleteRecordCrate, (json)req.params.id:" + JSON.stringify(req.params.id));
