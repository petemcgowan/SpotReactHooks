...
export const RecordCrateProvider = ({ children }) => {
  ...
  async function searchRecordCrate() {
    try {
      ...
      const res = await axios.post('/recordCrates/search', {"emailAddress": getEmail()}, config);
      console.log ("RecordCrateContext/State, searchRecordCrate():" + JSON.stringify(res.data.data));
  ...
  async function addRecordCrate(artists, releaseName, emailAddress) {
    const config = {
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    try {
      console.log("addRecordCrate, artists/releaseName/emailAddress:" + artists + releaseName + emailAddress);
      const res = await axios.post('/recordCrates/add', { emailAddress: emailAddress, artists: artists, releaseName: releaseName}, config);
  ...
  return (<RecordCrateContext.Provider value={{
